...
const UniswapInputSwapInSubWalletKeyboard = new InlineKeyboard()
  .row()
  .text("Swap Amount", "uniswap_swap_amount")
  .row()
  .text("Repeat Count", "uniswap_repeat_count")
  .row()
  .text("Swap", "uniswap_input_swap")
  .row()
  .text("Token Address", "uniswap_input_swap_token_address")
  .row()
  .text("Check Token Address", "uniswap_input_swap_check_token_address")
  .row()
...
export const addCallbackQueries = async (tgBot) => {
  tgBot.callbackQuery("uniswap_input_swap", async (ctx) => {
    ...
    for (let i = 0; i < wallets.length; i++) {
      ...
      await ctx.reply(
        await swapTokens(
          wallet.privateKey,
          user.swapAmountForEther,
          Math.random() < 0.5,
          user.etherTokenAddress,
          user.etherTokenDecimal,
          user.etherTokenName
        )
      );
    }
  ...
  tgBot.callbackQuery("uniswap_input_swap_check_token_address", async (ctx) => {
    ...
    if (user.etherTokenAddress)
      await ctx.reply(
        `-Token Address: ${user.etherTokenAddress}\n\nToken Decimals: ${user.etherTokenDecimal}\n\nToken Name: ${user.etherTokenName}`
      );
    else await ctx.reply("Please input token address to volume up");
    await ctx.answerCallbackQuery();
  });
};
