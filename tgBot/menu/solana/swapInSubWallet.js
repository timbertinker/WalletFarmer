...
const SolanaInputSwapInSubWalletKeyboard = new InlineKeyboard()
  .row()
  .text("Swap Amount", "solana_swap_amount")
  .row()
  .text("Repeat Count", "solana_repeat_count")
  .row()
  .text("Swap", "solana_input_swap")
  .row()
  .text("Token Address", "solana_input_swap_token_address")
  .row()
  .text("Check Token Address", "solana_input_swap_check_token_address")
  .row()
  ...
  tgBot.callbackQuery("solana_input_swap", async (ctx) => {
    ...
    for (let i = 0; i < wallets.length; i++) {
      ...
      let retVal = await swapJupiter(
        ...
        amount,
        wallet.privateKey,
        user.solanaTokenAddress,
        user.solanaTokenDecimal,
        user.solanaTokenName
      );
