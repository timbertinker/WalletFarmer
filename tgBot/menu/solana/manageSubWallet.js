...
const CAPTION_SOLANA_MANAGE_SUB_WALLET = "❓How many wallet would you like to create?\n💡We will create randomized wallet for you.";

const SolanaManageSubWalletKeyboard = new InlineKeyboard()
        .row().text("Create Sub Wallets", "solana_input_crerate_sub_wallets")
        .row().text("Deposit Amount", "solana_input_deposite_amount")
        .row().text("Deposit", "solana_input_deposite_sub_wallet").text("Wrap", "solana_input_wrap_sub_wallet")
        .row().text("Check Sub Wallets", "solana_input_check_sub_wallets")
        .row().text('Back','back_to_solana');

export const returnToManageSubWallet = async (tgBot, ctx) => {
...
export const addCallbackQueries = async (tgBot) => {
    ...
    tgBot.callbackQuery('solana_input_check_sub_wallets', async (ctx) => {
        var wallets = await SolanaWallet.find({ createdBy: ctx.from.username, isMain: false });
        if (wallets == null || wallets.length == 0)
            return await ctx.reply("There is no sub wallets to check");
        for (let i = 0; i < wallets.length; i++){
            let wallet = wallets[i];
            try {
